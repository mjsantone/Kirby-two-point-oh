...
- **Illustrated artifacts**: when `OPENAI_API_KEY` is set, Claude may embed generated images *inside* HTML artifacts via `<img src="/api/genimage?prompt=…">` (max 3, optional `&size=wide|tall`) — the server generates them on first load via the image model and caches them in memory. Downloading an artifact inlines these as data URIs so the `.html` stays self-contained.
