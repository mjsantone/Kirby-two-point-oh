...
A playful canvas where you drop up to **5 ingredients** — text snippets, images, PDFs, and previous artifacts, in any mix — arrange them as gooey metaballs, size each one for how much **influence** it should have, add a short directive ("make this into a retro arcade game"), and fuse them with **Claude Fable 5** into a single self-contained **HTML artifact**: a report, a game, a quiz, a slide deck, a choose-your-own-adventure, or whatever fits.
...
- **Frontend** (`public/`): vanilla JS. Each ingredient stores an explicit influence value; its normalized percentage drives blob **area** on a wide responsive size ramp. Selecting a blob reveals a compact `Use as` dropdown with a source-aware default — text as Content, images as Style, PDFs as Evidence, previous artifacts as Behavior, and repository modes as Behavior, Evidence, or Style — which the user can override to Content, Style, Behavior, Evidence, or Constraint. Influence controls how much the ingredient shapes the result; role controls how it contributes, and constraints remain mandatory regardless of percentage. Scroll on a blob (or use the ＋/－ toolbar) to change its influence. Dragging reshapes the composition, then a damped magnetic layout settles every blob into a connected cluster around the new release direction. At rest, the shared neck breathes with a subtle synchronized magnetic pulse while remaining continuously connected; the pulse pauses during drag and fusion and is disabled for reduced motion.
- **PDF ingredients**: upload a text-based PDF up to 12 MB and 100 pages, preview the extracted page outline, choose only the pages Fuse should use, and add the snapshot as Content, Evidence, or a Constraint. Capsules preserve the source filename, SHA-256 digest, document metadata, and page references; generated factual claims are instructed to cite the document title and page number. Extraction and prompt content are bounded, directional control characters and delimiter lookalikes are neutralized, and previews expire after ten minutes. Scanned PDFs currently return explicit OCR guidance rather than silently producing an empty ingredient.
- **Labs — public GitHub repositories (experimental)**: paste a public repository root URL, inspect a deterministic commit-pinned file proposal, adjust the selected files and intended use, then add it as one repository metaball. The server uses GitHub's public read APIs, never clones or executes code, skips binaries/generated dependencies/likely secrets, caps each excerpt and the full capsule, preserves file-path and blob-SHA provenance, caches responses for five minutes, and labels all repository content as untrusted source data in the model prompt.
...
| `GITHUB_TOKEN` | — | Optional server-only token to raise public GitHub API limits; does not grant users access to private repositories |
